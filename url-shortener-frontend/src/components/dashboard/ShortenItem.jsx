...
const ShortenItem = ({ originalUrl, shortUrl, clickCount, createdDate }) => {
  ...
  const subDomain = import.meta.env.VITE_REACT_SUB_DOMAIN.replace(
    /^https?:\/\//,
    ""
  );
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`flex-1 sm:space-y-1 max-w-full overflow-x-auto overflow-y-hidden `}
        >
          <div className="text-slate-900 pb-1 sm:pb-0   flex items-center gap-2">
            <a
              href={`${import.meta.env.VITE_REACT_SUB_DOMAIN}/${shortUrl}`}
              target="_blank"
        ...
        <div className="flex flex-1 sm:justify-end items-center gap-4">
          <CopyButton
            text={`${import.meta.env.VITE_REACT_SUB_DOMAIN}/${shortUrl}`}
          />
          <div
